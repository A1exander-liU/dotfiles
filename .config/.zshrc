...
####  Neofetch Daily ####
if [ ! -f ~/.neofetch_last_run ]; then
  touch ~/.neofetch_last_run
fi

last_run=$(cat ~/.neofetch_last_run)
now=$(date '+%Y-%m-%d')

if [[ "$now" != "$last_run" ]]; then
  neofetch
  echo "$now" > ~/.neofetch_last_run
fi
...
# colored-man-pages Configurations
